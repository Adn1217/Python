...
def tryLogin(userEntry, pswEntry):
...
# loginButton = Button(window, text="Ingresar", command=lambda: log.logIn(userEntry, pswEntry, infoText));
loginButton = Button(window, text="Ingresar", command=lambda: tryLogin(userEntry, pswEntry));
loginButton.grid(row=3, column=0, rowspan=1, padx=10, pady=10);

cancelButton = Button(window, text="Cancelar", command=window.quit);
